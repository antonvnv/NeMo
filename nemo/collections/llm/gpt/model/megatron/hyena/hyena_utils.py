...
class ParallelHyenaOperator(nn.Module):
    ...
    def forward_long(self, *, x1, x2, v, h, bias, inference_params):
    ...
    def forward_medium(self, *, x1, x2, v, h, bias, inference_params):
        import nemo.collections.llm.gpt.model.megatron.hyena.engine as engine
        def update_filter_state(filter_name, *, state):
            if not inference_params: return
            key = "{filter_name}_filter_state_dict"
    ...
    def forward(self, x1, x2, v, _hyena_use_cp=True, inference_params=None):
        ...
        if self.use_slow_heads:
            ...
            if with_state_m and self.operator_type == "hyena_medium_conv":
                ...
                # h: torch.Size([1920, 16])
                return self.forward_medium(x1=x1, x2=x2, v=v, h=h, bias=conv_bias, inference_params=inference_params)
            elif with_state_l and self.operator_type == "hyena":
                return self.forward_long(x1=x1, x2=x2, v=v, h=h, bias=conv_bias, inference_params=inference_params)
            else:
                pass
...
class ParallelShortHyenaOperator(nn.Module):
    ...
    def forward(self, x1, x2, v, inference_params, _hyena_use_cp=True):
        """
        Note:
            Input shapes: bs, seq_length, (num_groups, group_size)
            Output shapes: bs, seq_length, num_groups, group_size
